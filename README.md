...
查看解析会进入独立解析页面，流式解析和继续追问不会拉长原答题卡；可随时返回当前刷题位置。点击“获取相关题目”后进入独立相关练习页面。相关练习中的题库内题目答错后会立即加入错题集；每张相关题卡顶部都可收藏。联网拓展题首次收藏时会作为该题库的关联收藏持久保存，不会混入普通顺序或乱序模式。题库页面提供独立“收藏夹”入口，可查看、取消收藏并开始收藏练习。
