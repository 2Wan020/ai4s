...
支持顺序/乱序单选、顺序/乱序多选、错题重练。答题页左上角可收藏或取消收藏当前题目，收藏状态独立保存；错题集与错题练习会读取“答错题目”和“收藏题目”的并集，取消收藏不会移除仍有答错记录的题目。答题卡右下角可输入本次练习内的题号直接跳转，已作答状态和最近位置会保留。答题模式页面和答题页都提供“打乱选项顺序”勾选项；答题中切换后从下一道尚未展示的题开始生效，当前题及已经浏览的题目保持原有顺序。题目乱序与选项乱序可以独立选择；选项重排后正确答案字母会自动同步映射，该偏好会随题库和刷题进度一同保存。用户可按题库查看、练习、移出或清空错题集。每个导入文件会保存为一个独立题库文件夹，文件夹内的题目按单选/多选统计和筛选；导入题库可从题库卡片直接删除，删除时同步清理该题库的进度、错题与收藏记录。导入后每道题都会拆成独立题卡和独立选择界面。

相关练习中的题库内题目答错后会立即加入错题集；每张相关题卡顶部都可收藏。联网拓展题首次收藏时会作为该题库的关联收藏持久保存，不会混入普通顺序或乱序模式。题库页面提供独立“收藏夹”入口，可查看、取消收藏并开始收藏练习。
